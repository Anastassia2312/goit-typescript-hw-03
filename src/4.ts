...
яка генерується випадково при створенні об'єкта цього класу (наприклад Math.random()). Також цей клас повинен мати метод 
...
abstract class House {
  ...
  comeIn(person: Person) {
    if (this.door === true) {
      ...
      console.log("person can`t enter the house");
    }
  }
...
class MyHouse extends House {
  openDoor(key: Key) {
    if (key.getSignature() === this.key.getSignature()) {
      ...
      console.log("Door closed. You have a wrong key.");
    }
  }
}
